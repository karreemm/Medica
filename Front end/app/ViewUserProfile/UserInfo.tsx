  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        let url = "";
        if (role === "doctor") {
          url = `http://localhost:5221/api/Doctor/Doctor?did=${userId}`;
        } else if (role === "patient") {
          url = `http://localhost:5221/GetPatient?uid=${userId}`;
        } else if (role === "nurse") {
          url = `http://localhost:5221/api/Nurse/GetNurse?uid=${userId}`;
        }
